...
import com.example.boot06_article.dto.PasswordForm;
import com.example.boot06_article.model.MemberUser;
import com.example.boot06_article.service.MemberService;
...
public class HomeController {
    ...
    public String postMemberAdd(@Valid @ModelAttribute("member") MemberForm memberForm
        ...
        if (memberForm.getPassword() == null || memberForm.getPassword().trim().length() < 8) {
            bindingResult.rejectValue("password", "NotBlank"
                    , "패스워드를 8글자 이상 입력하세요");
        }
         */
